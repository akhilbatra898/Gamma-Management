...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO transit (carrier, route, documents_required, place_of_delivery) VALUES (%s, %s, %s, %s)",
...
$maxRows_Transit = 10;
$pageNum_Transit = 0;
if (isset($_GET['pageNum_Transit'])) {
  $pageNum_Transit = $_GET['pageNum_Transit'];
}
$startRow_Transit = $pageNum_Transit * $maxRows_Transit;

mysql_select_db($database_localhost, $localhost);
$query_Transit = "SELECT * FROM transit";
$query_limit_Transit = sprintf("%s LIMIT %d, %d", $query_Transit, $startRow_Transit, $maxRows_Transit);
$Transit = mysql_query($query_limit_Transit, $localhost) or die(mysql_error());
$row_Transit = mysql_fetch_assoc($Transit);

if (isset($_GET['totalRows_Transit'])) {
  $totalRows_Transit = $_GET['totalRows_Transit'];
} else {
  $all_Transit = mysql_query($query_Transit);
  $totalRows_Transit = mysql_num_rows($all_Transit);
}
$totalPages_Transit = ceil($totalRows_Transit/$maxRows_Transit)-1;
?>
<!DOCTYPE html>
<html lang="en">
   <head>
   ...
   <tbody>
          <?php do { ?>
            <tr>
              ...
              <td><?php echo $row_Transit['carrier']; ?></td>
              <td><?php echo $row_Transit['route']; ?></td>
              <td><?php echo $row_Transit['documents_required']; ?></td>
              <td><?php echo $row_Transit['place_of_delivery']; ?></td>
              <td><a href="update-transit.php?TranSR=<?php echo $row_Transit['TranSR']; ?>"> Update </a></td>
              <td><a href="delete-transit.php?TranSR=<?php echo $row_Transit['TranSR']; ?>"> Delete </a></td>
            </tr>
            <?php } while ($row_Transit = mysql_fetch_assoc($Transit)); ?>
            </tbody>
        </table>
                    ...
                    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
                      <table align="center">
                        ...
                        <tr valign="baseline">
                          ...
                          <td><input type="text" name="route" value="" size="32"></td>
                        </tr>
                        <tr valign="baseline">
                          <td nowrap align="right">Documents_required:</td>
                          <td><input type="text" name="documents_required" value="" size="32"></td>
                        </tr>
                        <tr valign="baseline">
                          <td nowrap align="right">Place_of_delivery:</td>
                          <td><input type="text" name="place_of_delivery" value="" size="32"></td>
                        </tr>
                        <tr valign="baseline">
